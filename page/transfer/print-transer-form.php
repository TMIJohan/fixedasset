<?php
	...
	if (isset($_POST['btntransfer'])) {
		...
		$query = oci_parse($conn, "SELECT TBLASSET.ASSETCODE, TBLASSET.ASSETDESC, TBLINV.INVSERIAL, TBLINV.INVMODEL, TBLINV.YEAR, 
			 TRANSFERFROM.DEPTCODE || ' - ' || TRANSFERFROM.DEPTNAME AS TRANSFERFORM, 
			 TRANSFERTO.DEPTCODE || ' - ' || TRANSFERTO.DEPTNAME AS TRANSFERTO, 
			 TBLINV.INVDATE
			 FROM TBLASSET INNER JOIN TBLINV ON TBLASSET.ASSETCODE = TBLINV.ASSETCODE 
			 INNER JOIN TBLDEPARTMENT TRANSFERFROM ON TBLINV.TRANFERFROM = TRANSFERFROM.DEPTCODE
			 INNER JOIN TBLDEPARTMENT TRANSFERTO ON TBLINV.TRANSFERTO = TRANSFERTO.DEPTCODE
			 WHERE INVTYPE = 'TRANSFER' AND TBLINV.DELETED = 0 AND TBLINV.INVCODE = '".$TransferCode."'
			"); 
		oci_execute($query);
		...
		$pdf->Cell(10, 10, ":");    
		$pdf->SetFont('Arial', '');
		$pdf->Cell(300, 10,  $row['TRANSFERFORM'], 'B', 'L');

		$pdf->SetFont('Arial', '', 10);
	    $pdf->SetY(240);
	    $pdf->Cell(170, 10, "Transfer to");
	    $pdf->SetFont('Arial', 'B');
		$pdf->Cell(10, 10, ":");    
		$pdf->SetFont('Arial', '');
		$pdf->MultiCell(300, 10,  $row['TRANSFERTO'], 'B', 'L');

		$pdf->SetFont('Arial', '', 10);
	    $pdf->SetY(280);
	    $pdf->Cell(420, 10, "Requested by,");
	    // $pdf->SetFont('Arial', '');
		// $pdf->Cell(150, 10, "Received by,");    
		$pdf->SetFont('Arial', '');
		$pdf->Cell(100, 10, "Approved by,");

		$pdf->SetFont('Arial', '', 10);
	    $pdf->SetY(340);
	    $pdf->Cell(420, 10, "Dept HOD");
	    // $pdf->SetFont('Arial', '');
		// $pdf->Cell(150, 10, "Dept HOD");    
		$pdf->SetFont('Arial', '');
		...
		// 	WHERE INVTYPE = 'TRANSFER' AND TBLINV.DELETED = 0 AND TBLASSETDEPR.MONTHID = '".$dateinv."'
		// 	 AND TBLINV.INVCODE = '".$TransferCode."'
		//  	"); 
